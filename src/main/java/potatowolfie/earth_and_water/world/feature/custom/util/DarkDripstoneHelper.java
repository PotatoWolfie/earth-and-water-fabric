package potatowolfie.earth_and_water.world.feature.custom.util;
...
public class DarkDripstoneHelper {
    ...
    public static boolean canGenerate(WorldAccess world, BlockPos pos) {
        return world.testBlockState(pos, potatowolfie.earth_and_water.world.feature.custom.util.DarkDripstoneHelper::canGenerate);
    }

    public static boolean canGenerateOrLava(WorldAccess world, BlockPos pos) {
        return world.testBlockState(pos, potatowolfie.earth_and_water.world.feature.custom.util.DarkDripstoneHelper::canGenerateOrLava);
    }

    protected static void getDarkDripstoneThickness(Direction direction, int height, boolean merge, Consumer<BlockState> callback) {
        if (height >= 3) {
    ...
    public static void generatePointedDarkDripstone(WorldAccess world, BlockPos pos, Direction direction, int height, boolean merge) {
        if (canReplace(world.getBlockState(pos.offset(direction.getOpposite())))) {
            BlockPos.Mutable mutable = pos.mutableCopy();
            getDarkDripstoneThickness(direction, height, merge, (state) -> {
                if (state.isOf(ModBlocks.POINTED_DARK_DRIPSTONE)) {
                    state = (BlockState)state.with(PointedDarkDripstoneBlock.WATERLOGGED, world.isWater(mutable));
                }
    ...
    public static boolean generateDarkDripstoneBlock(WorldAccess world, BlockPos pos) {
        BlockState blockState = world.getBlockState(pos);
        if (blockState.isIn(BlockTags.DRIPSTONE_REPLACEABLE_BLOCKS)) {
            world.setBlockState(pos, ModBlocks.DARK_DRIPSTONE_BLOCK.getDefaultState(), 2);
            return true;
        } else {
            return false;
        }
    ...
    private static BlockState getState(Direction direction, Thickness thickness) {
        return (BlockState)((BlockState)ModBlocks.POINTED_DARK_DRIPSTONE.getDefaultState().with(PointedDarkDripstoneBlock.VERTICAL_DIRECTION, direction)).with(PointedDarkDripstoneBlock.THICKNESS, thickness);
    }
    ...
    public static boolean canReplace(BlockState state) {
        return state.isOf(ModBlocks.DARK_DRIPSTONE_BLOCK) || state.isIn(BlockTags.DRIPSTONE_REPLACEABLE_BLOCKS);
    }
