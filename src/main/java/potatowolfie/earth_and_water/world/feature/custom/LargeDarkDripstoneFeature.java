...
import potatowolfie.earth_and_water.world.feature.custom.util.DarkDripstoneHelper;
...
public class LargeDarkDripstoneFeature extends Feature<LargeDarkDripstoneFeatureConfig> {
    ...
    public boolean generate(FeatureContext<LargeDarkDripstoneFeatureConfig> context) {
        StructureWorldAccess structureWorldAccess = context.getWorld();
        BlockPos blockPos = context.getOrigin();
        LargeDarkDripstoneFeatureConfig largeDarkDripstoneFeatureConfig = context.getConfig();
        Random random = context.getRandom();
        if (!DarkDripstoneHelper.canGenerate(structureWorldAccess, blockPos)) {
            return false;
        } else {
            Optional<CaveSurface> optional = CaveSurface.create(structureWorldAccess, blockPos, largeDarkDripstoneFeatureConfig.floorToCeilingSearchRange, DarkDripstoneHelper::canGenerate, DarkDripstoneHelper::canReplaceOrLava);
            if (!optional.isEmpty() && optional.get() instanceof CaveSurface.Bounded) {
                ...
                if (bounded.getHeight() < 4) {
                    ...
                    int i = (int)((float)bounded.getHeight() * largeDarkDripstoneFeatureConfig.maxColumnRadiusToCaveHeightRatio);
                    int j = MathHelper.clamp(i, largeDarkDripstoneFeatureConfig.columnRadius.getMin(), largeDarkDripstoneFeatureConfig.columnRadius.getMax());
                    int k = MathHelper.nextBetween(random, largeDarkDripstoneFeatureConfig.columnRadius.getMin(), j);
                    DarkDripstoneGenerator darkDripstoneGenerator = createGenerator(blockPos.withY(bounded.getCeiling() - 1), false, random, k, largeDarkDripstoneFeatureConfig.stalactiteBluntness, largeDarkDripstoneFeatureConfig.heightScale);
                    DarkDripstoneGenerator darkDripstoneGenerator2 = createGenerator(blockPos.withY(bounded.getFloor() + 1), true, random, k, largeDarkDripstoneFeatureConfig.stalagmiteBluntness, largeDarkDripstoneFeatureConfig.heightScale);
    ...
    private static DarkDripstoneGenerator createGenerator(BlockPos pos, boolean isStalagmite, Random random, int scale, FloatProvider bluntness, FloatProvider heightScale) {
        return new DarkDripstoneGenerator(pos, isStalagmite, scale, (double)bluntness.get(random), (double)heightScale.get(random));
    }
    ...
    private void testGeneration(StructureWorldAccess world, BlockPos pos, CaveSurface.Bounded surface, WindModifier wind) {
        ...
        for(BlockPos.Mutable mutable = pos.withY(surface.getFloor() + 2).mutableCopy(); mutable.getY() < surface.getCeiling() - 1; mutable.move(Direction.UP)) {
            ...
            if (DarkDripstoneHelper.canGenerate(world, blockPos) || world.getBlockState(blockPos).isOf(ModBlocks.DARK_DRIPSTONE_BLOCK)) {
