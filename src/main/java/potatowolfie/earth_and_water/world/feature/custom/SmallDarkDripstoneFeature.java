...
public class SmallDarkDripstoneFeature extends Feature<SmallDarkDripstoneFeatureConfig> {
    ...
    public boolean generate(FeatureContext<SmallDarkDripstoneFeatureConfig> context) {
        ...
        SmallDarkDripstoneFeatureConfig config = context.getConfig();
        ...
        if (optional.isEmpty()) {
            ...
            generateDripstoneBlocks(worldAccess, random, blockPos2, config);
            int i = random.nextFloat() < config.chanceOfTallerDripstone && DarkDripstoneHelper.canGenerate(worldAccess.getBlockState(blockPos.offset(optional.get()))) ? 2 : 1;
            DarkDripstoneHelper.generatePointedDarkDripstone(worldAccess, blockPos, optional.get(), i, false);
            return true;
        }
    }
